...
def tuition_by_course():
    print(f"{'Course Code':<15}{'# of Stu':<10}Tuition Generated")
    print("---------------------------------------------------------------")
    
    # Loop through all keys and values in the courses dictionary
    
    for k,v in courses.items():
        # Loop through all values in the students dictionary(values are lists)
        students_in_k = 0
        for classlist in students.values():
            # Check tosee if the student is in class k
            if k in classlist:
                students_in_k += 1
        # All class list have been checked - inner for loop breaks
        # Go get the value for the tuition of class k
        tuition_for_k = courses[k]['tuition']
        total_tuition_for_k = students_in_k * tuition_for_k
        # Display the values
        print(f"{k:<15}{students_in_k:<10}${total_tuition_for_k:,.2f}")
